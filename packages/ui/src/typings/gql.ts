...
export type GetQueueMetricsQuery = { metrics?: Maybe<Array<(
    ...
    & { counts: Pick<QueueJobsCounts, 'prioritized' |'waiting' | 'active' | 'completed' | 'failed' | 'delayed' | 'paused'> }
  )>> };
...
export type GetQueuesQuery = { queues?: Maybe<Array<(
    ...
    & { jobsCounts: Pick<QueueJobsCounts, 'prioritized' |'waiting' | 'active' | 'completed' | 'failed' | 'delayed' | 'paused'> }
  )>> };
